...
class NiFiClient:
	...
	def update_processor(
		...
		if bulletin_level is not None:
			# Noticed inconsistency in NiFi API, sometimes bulletinLevel is top-level, sometimes in config
			# Usually top-level for processor updates in newer versions
			component["config"] = component.get("config", {})
			component["config"]["bulletinLevel"] = bulletin_level
	...
	def update_connection(
		...
		"""Update a connection. Requires NIFI_READONLY=false."""
		return self._put(
	...
	def create_funnel(self, pg_id: str, position_x: float = 0.0, position_y: float = 0.0) -> Dict[str, Any]:
		"""Create a funnel in a process group. Requires NIFI_READONLY=false."""
		return self._post(
			f"process-groups/{pg_id}/funnels",
			{
				"revision": {"version": 0},
				"component": {
					"position": {"x": position_x, "y": position_y}
